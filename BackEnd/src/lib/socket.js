...
io.on("connection", (socket) => {
    ...
    debug("✅ User connected:", userId, socket.id);
    ...
    debug("📢 Online users broadcasted:", Object.keys(userSocketMap).length, "users");

    // -------------------------
    // Private Message
    // -------------------------
    socket.on("privateMessage", ({ toUserId, message }) => {
        debug(`📨 PRIVATE_MESSAGE event received from ${userId} to ${toUserId}`);
        const payload = {
            ...message,
            senderId: userId
        };
        ...
        debug(`📨 PRIVATE_MESSAGE emitted to both ${toUserId} and ${userId}`);
    });
    ...
    socket.on("typing", ({ toUserId }) => {
        debug(`🔵 TYPING event received: ${userId} is typing to ${toUserId}`);
        io.to(toUserId).emit("typing", { senderId: userId });
        debug(`🔵 TYPING event emitted to user ${toUserId}`);
    });

    socket.on("stopTyping", ({ toUserId }) => {
        debug(`🔴 STOP_TYPING event received: ${userId} stopped typing to ${toUserId}`);
        io.to(toUserId).emit("stopTyping", { senderId: userId });
        debug(`🔴 STOP_TYPING event emitted to user ${toUserId}`);
    });

    socket.on("groupTyping", ({ groupId, userName }) => {
        debug(`🔵 GROUP_TYPING event received: ${userName} (${userId}) is typing in group ${groupId}`);
        // Broadcast to everyone to ensure visibility in GroupList
        io.emit("groupTyping", { groupId, userId, userName });
        debug(`🔵 GROUP_TYPING event broadcasted to all users`);
    });

    socket.on("groupStopTyping", ({ groupId, userName }) => {
        debug(`🔴 GROUP_STOP_TYPING event received: ${userName} (${userId}) stopped typing in group ${groupId}`);
        io.emit("groupStopTyping", { groupId, userId, userName });
        debug(`🔴 GROUP_STOP_TYPING event broadcasted to all users`);
    });
    ...
    socket.on("disconnect", () => {
        debug("User disconnected:", userId, socket.id);
